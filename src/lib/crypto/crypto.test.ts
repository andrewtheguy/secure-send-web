...
describe('ECDH Utils', () => {
  ...
  it('should derive equivalent shared secrets for two peers (non-extractable)', async () => {
    ...
    const aliceSharedKey = await deriveSharedSecretKey(
      alice.privateKey,
      bob.publicKeyBytes,
    )
    const bobSharedKey = await deriveSharedSecretKey(
      bob.privateKey,
...
describe('Stream Crypto', () => {
  ...
  it('should authenticate chunk index metadata', async () => {
    ...
    await expect(
      decryptChunk(key, parsed.encryptedData, parsed.chunkIndex),
    ).rejects.toThrow()
  })
