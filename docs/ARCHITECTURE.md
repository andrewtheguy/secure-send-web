...
Two **separate** one-way derivations of the PIN, sharing the same KDF but with different salts so they are independent values. Both expose nothing about the PIN itself (`computePinHint` / `computePinHintFromKey` / `computePinFingerprint` in `src/lib/crypto/pin.ts`). Both truncate to the first **16 hex characters (64 bits)** of `PBKDF2-SHA256(pin, …, iterations = 600,000)`, and both salts are distinct from the per-transfer random salt used for the content key, so neither is ever equal to the key-derivation output.
...
- **Fingerprint — stable visible checksum** (`computePinFingerprint`): salt = `"secure-send:pin-hint:v1"` (the fixed `PIN_HINT_SALT`, **no** time bucket). Displayed to both parties (grouped as `XXXX-XXXX-XXXX-XXXX` by `formatPinHint`) as a one-way checksum; the receiver computes it locally after entering the PIN and matching fingerprints confirm both sides derived the same secret. It is **never published to relays** — it exists only for human visual comparison — so it is kept time-independent: both sides always display the same value even across an hour-bucket rollover. It cannot be reversed to recover the PIN or used to decrypt any data.
- **Why two values**: the wire hint benefits from rotating (it sits on a public relay as a potential correlator), while the fingerprint must stay constant (a rotating fingerprint would intermittently mismatch between sender and receiver near a bucket boundary). The fixed public salt also defeats generic precomputed-hash (rainbow table) attacks in both cases.
- **Not the security boundary**: neither value gates confidentiality — the hint only *locates* the event and the fingerprint only *confirms* the PIN. Confidentiality rests entirely on the PIN-derived AES key. Reversing either to a PIN requires brute-forcing the full PIN space (~65.6 bits, Nostr) at 600,000 iterations per guess — the same cost as attacking the ciphertext directly, so they grant an attacker no shortcut.
