...
- **PIN authenticates, then expires (Nostr)**: The sender mints a fresh 10-character PIN (~45 bits) every 2 minutes and honors only the 2 most recent. The receiver locates the rendezvous event via a one-way rotating hint tag, restricting its lookup to the current and immediately previous rotation buckets. The PIN seals a mutual claim/confirm challenge-response that binds both sides' ECDH public keys, defeating relay man-in-the-middle. The first verified claim locks the transfer to that receiver; the PIN itself is never transmitted
...
- **Expiration windows**: Each PIN is honored for 4 minutes; rendezvous events carry a matching NIP-40 expiration tag for relays that honor it, and the sender stops waiting after 30 minutes (a resource backstop — rotation, not the wait window, bounds PIN exposure)
