...
- **File or folder transfer**: Send a file, or a ZIP archive created from multiple files/a folder, up to 2GB — payloads over 100MB stream through on-disk OPFS scratch storage at every stage (archiving, sending, receiving), so large transfers are not memory-bound; payloads of 100MB or less are buffered in memory and never touch disk. See [Browser Requirements](#browser-requirements)
...
Transfers over 100MB stage in-flight data in the origin-private file system (OPFS), which requires:
...
Support is feature-detected at runtime; on an unsupported browser, starting a transfer over 100MB fails with a clear error rather than degrading. Transfers of 100MB or less are buffered in memory and do not need OPFS (a secure context is still required for the Web Crypto API).
